...
// drpy3 仓库内置版（自 E:/gitwork/drpy-node/libs_drpy/htmlParser.js 移植，仅头部本地化）：
//   log → 本地实现；urljoin → WHATWG URL 等价实现；jsonpathplus → 同目录文件。
// 解析逻辑与 drpy-node 生产实现逐字一致（勿手改解析行为，升级请对照上游）。
import * as cheerio from 'cheerio';
import './jsonpathplus.min.js';

const log = (...args) => console.log('[htmlParser]', ...args);
