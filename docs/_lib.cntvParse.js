//import './lib/wasm.js';
import './_lib.cctv.worker.new.js';
// ==================== 常量定义 ====================
const TS_PACKET_SIZE = 188;
const DEFAULT_VIDEO_PID = 256;

// 动态检测视频 PID：统计包数量，视频流通常包数最多
...
async function decryptH264InPlace(h264Data) {
    ...
    // 直播流没有 type 25 NALU，默认解密；点播流有 type 25，由其控制是否解密
...
async function Parse_TS(buffer) {
    ...
    const originalTS = Buffer.from(new Int8Array(buffer));

    // 动态检测视频 PID（不同频道可能使用不同的 PID）
    ...
    return originalTS.buffer;
}


globalThis.Parse_TS = Parse_TS;
